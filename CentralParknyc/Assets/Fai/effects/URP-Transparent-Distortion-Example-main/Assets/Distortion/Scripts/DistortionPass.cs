...
class DistortionPass : ScriptableRenderPass
{
    ...
    {
        if (passData == null || passData.tmpRT == null)
            return;
        ...
        CommandBuffer cmd = CommandBufferPool.Get("Distortion Pass");
        ...
        using (new ProfilingScope(cmd, passData.profilingSampler))
        {
            ...
            cmd.Blit(source, passData.tmpRT);
            cmd.SetGlobalTexture(k_GlobalFullSceneColorTexture, passData.tmpRT);
